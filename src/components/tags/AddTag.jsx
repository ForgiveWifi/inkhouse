...
import { uploadFirebase } from "../../utils/firebaseFunctions";
...
function AddTag({missingTags, toggleChange, close}) {
  ...
  async function postTag(design, ref) {
    try {
      ...
      const post = await axios.post(`${process.env.REACT_APP_API_URL}/tag`, {size: size, design: design}, {
        headers: {
  ...
  return (
    <>
      <div className="background1 flexbox-column" style={{ padding: 20 }}>
        <h1>Add Tag</h1>
      ...
      <ImageUpload />
        ...
        <div id="neck-preview" className="flexbox full-width" style={{ position: "relative", gap:10 }}>
          <div className="white-border radius10" style={{ width: small ? 240 : 320 , height: 240}}>
